...
[Library]
public partial class MovementController : MovementComponent
{
	...
	public override void Simulate( IClient cl )
	{
		...
		*/
		...
		if ( Swimming || IsTouchingLadder )
		{
			...
			WishVelocity *= Entity.ViewAngles.ToRotation();
		}
		else
		{
	...
	public virtual void CheckDuck()
	{
		...
		if ( IsDucking )
		{
			...
			delta -= DuckAmount;
			SetTag( "ducked" );
			if ( Entity.GroundEntity is null )
			{
				Entity.Position += Vector3.Up * (delta * Entity.Scale);
		...
		Entity.EyeLocalPosition = Entity.EyeLocalPosition.WithZ( EyeHeight + (DuckAmount) );
	...
	void DuckFrameSimulate()
	{
		...
		Entity.EyeLocalPosition = Entity.EyeLocalPosition.WithZ( EyeHeight + LocalDuckAmount );
	}
	...
	void RestoreGroundAngles()
	{
		...
		Entity.ViewAngles -= (PreviousViewAngles.Value.ToRotation() * worldTrnsView.Rotation.Inverse).Angles().WithPitch( 0 ).WithRoll( 0 );
	}
	void SaveGroundAngles()
	{
		...
		GroundTransformViewAngles = Entity.GroundEntity.Transform.ToLocal( new Transform( Vector3.Zero, Entity.ViewAngles.ToRotation() ) );
		PreviousViewAngles = Entity.ViewAngles;
	}
